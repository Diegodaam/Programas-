...
int main(){
	...
	for(int i = 0; i < (int)enemies.size(); i++){
		for(int j = 0; j < (int)enemies[i].size(); j++){
			if(i==0){
				...
				sectionSpritesheet = Vector2f(0,45+(8*4+4));
			}else if(i<5){
				sectionSpritesheet = Vector2f(0,54+(8*4+4)*2);
			}else if(i<7){
				sectionSpritesheet = Vector2f(0,63+(8*4+4)*3);
			}
	...
	while(window.isOpen()){
		...
		if(!player.Vivo()){ system("cls"); cout<<"Perdiste\n"; window.close();}
...
void UpdatePlayer(Player &player, Bullet &bulletPlayer){
		...
		if(player.Shoot()&&!bulletActive){
			Bullet bullet(player.Pos().x+24,player.Pos().y+12,spritesheet,IntRect(13*8+16,6*8+6,8,8),-30);
			bulletPlayer = bullet;
			bulletActive=true;
		}
}
...
void UpdateEnemies(vector<vector<Enemie>> &enemies){
	...
	if(timer>=cadencia&&(int)enemies[0].size()>0){
		...
		Bullet bullet = Bullet(enemies[0][enem].Pos().x+9,enemies[0][enem].Pos().y+24,spritesheet,IntRect(13*8+8,8*2+2,8,8),15);
